...
# Explicitly configure the libusb backend for PyUSB to use libusb.
# On Windows this will use libusb 0.1 32bit (libusb 1.0 causes enumeration errors)
# located in the local libusb0_x86.dll file.
# On Mac OSX this will use libusb 1.0 64bit located in the local libusb-1.0.dylib file.
# On other platforms like Linux it will look for a system installed libusb 1.0.
backend = None
if platform.system() == 'Windows':
    backend = usb.backend.libusb0.get_backend(find_library=lambda x: "libusb0_x86.dll")
elif platform.system() == 'Darwin':
    backend = usb.backend.libusb1.get_backend(find_library=lambda x: "libusb-1.0.dylib")
else:
